...
export function AIMessage({ content, onRemix, remixIteration = 0 }: AIMessageProps) {
  ...
  const Section = ({ icon: Icon, title, content: text, sectionKey }: { icon: any; title: string; content: string; sectionKey: string }) => (
      ...
      <div className="mb-6 animate-fade-in">
        <div className="max-w-3xl glass-card p-6 space-y-6">
            ...
            <>
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => copyToClipboard(content.hashtags.map((tag: string) => `#${tag}`).join(' '), 'HASHTAGS')} className="h-7 px-2">
          ...
          <div className="flex gap-2">
            <Button onClick={copyAll} variant="outline" className="flex-1 h-10 font-semibold border-primary/20 hover:bg-primary/10">
              <Copy className="mr-2 h-4 w-4" />
              Copy Everything
            </Button>
            {onRemix && (
              <Button onClick={() => onRemix(remixIteration + 1)} variant="outline" className="h-10 px-4 font-semibold border-accent/40 hover:bg-accent/10" title="Remix this content">
